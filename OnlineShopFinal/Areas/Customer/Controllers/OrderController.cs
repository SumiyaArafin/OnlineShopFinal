...
namespace OnlineShopFinal.Areas.Customer.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public OrderController(ApplicationDbContext db, UserManager<IdentityUser> userManager, IPdfSharpService pdfService)
        {
        ...
        public IActionResult Index(int? page)
        {
            ...
            return View(_db.Orders.Include(c => c.OrderDetails).ToList());
        }
                ...
                if (to != null)
                {
                    try {
                        ...
                        if (products != null)
                        {
                            ...
                            {
                                var pro = _db.PurchaseOrderLineItems.Include(c => c.Product).FirstOrDefault(c => c.Id == product.ProductId);
                                ...
                                orderDetails.Price = product.UnitPrice;
                                ...
                                if (B >= A)
                                {
                                    n = B - A;
                                    pro.Quantity = n;
                                    _db.PurchaseOrderLineItems.Update(pro);
        ...
        private void DemonstrateTableAlignment(Document document)
        {
            ...
            foreach (var product in products)
            {
                decimal single = product.UnitPrice * product.Quantity;
                total = total + single;
            ...
            decimal a = (decimal)(20.00 / 100.00);
        ...
        public void DemonstrateSimpleTable(Document document)
        {
            ...
            foreach (var product in products)
            {
                ...
                row.Cells[1].AddParagraph(product.ProductName.ToString());
                row.Cells[2].AddParagraph(product.UnitPrice.ToString());
                row.Cells[3].AddParagraph(product.Quantity.ToString());
                row.Cells[4].AddParagraph((product.UnitPrice * product.Quantity).ToString());
